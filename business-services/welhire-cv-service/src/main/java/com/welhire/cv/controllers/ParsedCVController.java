...
public class ParsedCVController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<ApiResponse<ParsedCandidateCV>> getById(
            @PathVariable("id") String id) {
        ParsedCandidateCV pcv = cVParsingService.getById(id);
        return ResponseEntity.ok(ApiResponse.success("Found", pcv));
    }


    @GetMapping("/search")
    public ResponseEntity<ApiResponse<Page<ParsedCandidateCV>>> search(
        ...
        Page<ParsedCandidateCV> result = cVParsingService.searchBy(allParams, pg);
